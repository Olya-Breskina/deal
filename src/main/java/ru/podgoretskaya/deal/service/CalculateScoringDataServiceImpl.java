...
public class CalculateScoringDataServiceImpl implements CalculateScoringDataService {
    ...
    public void calculateConditions(FinishRegistrationRequestDTO model, Long applicationId) {
        ...
        ApplicationEntity applicationEntity = applicationRepo.findById(applicationId).get();
        ClientEntity clientEntity = clientRepo.findById(applicationId).get();
        CreditEntity creditEntity = creditRepo.findById(applicationId).get();
        ScoringDataDTO scoringDataDTO = buildScoringData(model, clientEntity, creditEntity, applicationEntity);
    ...
    private ScoringDataDTO buildScoringData(FinishRegistrationRequestDTO model, ClientEntity clientEntity, CreditEntity creditEntity, ApplicationEntity applicationEntity) {
        ...
        log.info("заполнение scoringDataDTO. Параметры: \"" + scoringDataDTO.toString());
